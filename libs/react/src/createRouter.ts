...
export function matchRelayRoutes(
  ...
  return matchRoutes(
    ...
    pathname
  ) as unknown as MatchedRelayRoute[]
}
...
export default function createRouter(
  ...
  const cleanup = history.listen(({ location }: Update) => {
  ...
  const context: RoutingContextValue = {
    history,
    get() {
      return currentEntry
    },
    preloadCode(pathname) {
      ...
      const matches = matchRelayRoutes(routes, pathname)
...
function matchRoute(
  ...
  const matchedRoutes = matchRelayRoutes(routes, location.pathname)
  console.log({ routes, matchedRoutes })
 ...
 */
function prepareMatches(matches: MatchedRelayRoute[]): RouterEntryData[] {
