...
import { RouterEntryData } from './RoutingTypes'
...
export function RouterRenderer() {
  ...
  return (
    <RouteErrorBoundary>
      <Suspense fallback={'Loading route...'}>{routeComponent}</Suspense>
    </RouteErrorBoundary>
  )
}
 ...
 */
const RouteComponent: React.FC<RouterEntryData> = (props) => {
  const Component = props.component.read()
  const { prepared } = props
  return <Component {...prepared} children={props.children} />
}
